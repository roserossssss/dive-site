...
export default function MedicalManagement() {
  ...
  return (
    <div className="flex-1 p-3 sm:p-5 pt-2 relative">
      {/* Header Section */}
      ...
      {/* Table Section */}
			<div
				className="rounded-3xl overflow-hidden mt-5 sm:mt-7 bg-[#D9E7EC] shadow-md pt-2 pb-6 w-full"
				style={{ overflow: "hidden" }}
			>
				{recordList.length > 0 ? (
					<div
						className="overflow-x-auto min-h-[50vh] sm:min-h-[60vh] lg:min-h-[70vh]"
					...
					>
						<table className="w-full text-center text-[#001526] mt-1 table-auto sm:table-fixed text-xs sm:text-sm md:text-base">
							<thead className="bg-[#D9E7EC] border-b-2 border-gray-400">
								<tr>
									<th className="pl-6 sm:pl-10 pr-0 py-4 w-8 sm:w-12 text-left">
										<input
											type="checkbox"
									...
									<th className="px-2 sm:px-4 py-4 font-semibold text-xs sm:text-sm md:text-base w-[80px] sm:w-[120px]">
										User ID
									</th>
									<th className="px-2 sm:px-4 py-4 font-semibold text-xs sm:text-sm md:text-base w-[100px] sm:w-[180px]">
										Name
									</th>
									<th className="px-2 sm:px-4 py-4 font-semibold text-xs sm:text-sm md:text-base w-[80px] sm:w-[110px]">
										Age
									</th>
									<th className="px-2 sm:px-4 py-4 font-semibold text-xs sm:text-sm md:text-base w-[80px] sm:w-[110px]">
										Gender
									</th>
									<th className="px-2 sm:px-4 py-4 font-semibold text-xs sm:text-sm md:text-base w-[100px] sm:w-[130px]">
										Status
									</th>
									<th className="pr-2 sm:pr-10 pl-0 py-4 font-semibold text-xs sm:text-sm md:text-base w-[30px] text-right"></th>
								</tr>
							</thead>
							<tbody className="bg-[#D9E7EC] text-[#001526]">
								{currentPageRecords.map((record, index) => (
									...
									>
										<td className="pl-6 sm:pl-10 pr-2 py-4 w-8 sm:w-12 text-left">
											<input
												type="checkbox"
												className="appearance-none w-4 h-4 border-2 border-[#001526] rounded-full bg-[#D9E7EC] checked:bg-[#001526] checked:border-[#001526] focus:ring-2 focus:ring-[#001526] transition-colors"
										...
										<td className="px-1 sm:px-4 py-4 text-xs sm:text-sm md:text-base">
											{record.name}
										</td>
										<td className="px-1 sm:px-4 py-4 text-xs sm:text-sm md:text-base">
											{record.age}
										...
										<td className="px-1 sm:px-4 py-4 text-xs sm:text-sm md:text-base">
											<span
												className={`px-2 py-1 md:px-8 md:py-[5px] rounded-full font-semibold ${getStatusColor(
													record.status
												)}`}
											>
												{record.status}
										...
										<td className="pr-2 sm:pr-4 pl-0 py-4 w-12 sm:w-16 text-right relative">
											...
											{dropdownRecord === index && (
												<div className="absolute right-4 sm:right-14 mt-3 w-24 sm:w-36 top-2 bg-[#2C7DA0] text-white font-medium rounded-xl pt-3 pb-[0.55rem] px-2 z-10">
													<button
					...
					<div className="flex items-center justify-center w-full h-full text-center">
						<div>
							<p className="text-sm sm:text-lg font-bold text-[#001526] mt-3">
								No Records Found
							</p>
							<p className="text-xs sm:text-sm text-[#001526] mt-1">
