...
export default function Sidebar() {
    ...
    return (
        <>
            ...
            <div className={`fixed top-0 left-0 h-full w-64 md:w-72 flex flex-col text-black z-50 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
                ...
                <aside className="rounded-r-3xl bg-[#001526] flex-1 overflow-y-auto">
                                    ...
                                    <ul className="mb-14 mt-4 space-y-2">
                                        {category.list.map((item) => (
                                            <li key={item.title} className="relative text-center">
                                                ...
                                                >
                                                    <span 
                                                        className={`absolute left-0 top-0 h-full w-8 rounded-r-lg ${pathname === item.path ? "bg-[#2C7DA0] block" : "hidden"}`}
                                                    ></span>
